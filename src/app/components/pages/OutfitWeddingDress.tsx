...
const weddingDresses: OutfitItem[] = [
  {
    id: 1,
...
const categories = [
  { id: "all", name: "Tất cả", count: weddingDresses.length },
  { id: "classic", name: "Váy công chúa", count: weddingDresses.filter(item => item.category === "princess").length },
  { id: "vintage", name: "Vintage", count: 2 },
  { id: "modern", name: "Hiện đại", count: 1 }
];

export function OutfitWeddingDress() {
  const [activeTab, setActiveTab] = useState("all");
  const [hoveredItem, setHoveredItem] = useState<number | null>(null);

  const filteredDresses = activeTab === "all" 
    ? weddingDresses 
    : weddingDresses.filter(item => {
        if (activeTab === "classic") return item.category === "princess";
        if (activeTab === "vintage") return item.name.includes("Vintage") || item.name.includes("Classic");
        if (activeTab === "modern") return item.name.includes("Ethereal");
        return true;
      });
    ...
    <div className="pt-16">
      ...
      <section className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white max-w-4xl px-4">
          ...
          >
            <Badge className="mb-6 bg-white/10 text-white border-white/20">WEDDING DRESS</Badge>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-light mb-4">Wedding Dress Collection</h2>
            <p className="text-gray-600 max-w-2xl mx-auto">Bộ sưu tập váy cưới với nhiều phong cách từ cổ điển đến hiện đại</p>
          </div>
      ...
      <section className="py-20 bg-gradient-to-r from-amber-50 to-orange-50">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-light mb-6">Đặt lịch thử váy cưới</h2>
          <p className="text-gray-600 mb-8 max-w-2xl mx-auto">
            Hãy đến studio để trải nghiệm trực tiếp những mẫu váy cưới tuyệt đẹp này. 
            Đội ngũ stylist chuyên nghiệp sẽ tư vấn giúp bạn chọn được chiếc váy hoàn hảo.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
